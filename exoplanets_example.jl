...
# ╔═╡ 68f0848e-7a53-11ec-2b86-c36cf6af5b2f
using CSV, DataFrames, HTTP, FITSIO, DataFramesMeta, Plots

# ╔═╡ 04716fe6-20a6-4d01-9709-f54349aa99c8
f = FITS("data/TESS/tess2018206045859-s0001-0000000261136679-0120-s_lc.fits")

# ╔═╡ e8f3ca20-2102-42f9-8248-2e532632e7f4
df_all = f[2] |> DataFrame

# ╔═╡ add0fb19-94f4-427c-802d-af943ae880b6
describe(df_all)

# ╔═╡ 19bbf4cd-983a-40ab-bf68-e60633027f8a
df = @chain df_all begin
	@select(:TIME, :PDCSAP_FLUX)
	filter(row -> !(isnan(row.TIME) && isnan(row.PDCSAP_FLUX)), _)
end

# ╔═╡ 75146d25-77dc-423a-994c-9be6c0d52cfd
plotly()

# ╔═╡ fb0301ea-493f-4f72-bab9-89ad07448a5b
scatter(df.TIME, df.PDCSAP_FLUX, msw=0.0)
