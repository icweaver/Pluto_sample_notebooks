...
# ╔═╡ b15a6c4a-70be-11ec-0393-abfd46b3fa20
begin
	using AlgebraOfGraphics, CairoMakie # Plotting framework and library
	using DataFrames, DataFrameMacros # Dataframes and convenience functions 
	using Random # Make random stuff
	using PlutoUI # Easy widgets and more 🌈
end

# ╔═╡ 22f59dc2-f2f8-4cfd-b2bf-c6db0b102810
using PythonCall, CondaPkg

# ╔═╡ a84aa779-86e5-4417-a697-f18e7ff31c27
md"""
# Quick data viz example 📊

$(TableOfContents())
"""

# ╔═╡ 8d890b9e-360b-46f9-b60e-f4a22ce5c651
set_aog_theme!()

# ╔═╡ 9cd08d71-e286-4dc5-8a44-e401fd728eaf
md"""
## Make some data
"""

# ╔═╡ ba57767c-e5bf-487c-9d5f-d22dbf890c57
df_unprocessed = let
	areas = ["South West", "Edinburgh", "London", "Morecombe"]
	N = length(areas)
	DataFrame(
		Area = areas,
		ER_2005 = rand(1:100, N),
		ER_2015 = rand(1:100, N),
		ER_2025 = rand(1:100, N),
		something = map(_->randstring(), 1:N),
	)
end

# ╔═╡ c4a0d674-3e5e-42fe-91e4-88512fcdee68
df = @subset(
	df_unprocessed,
	in(["South West", "London"]).(:Area)
)[:, r"Area|ER_"]

# ╔═╡ a0fe38da-64a6-4623-8d0b-73d5fb123e09
_df_stacked = stack(df, r"ER_")

# ╔═╡ 0b216abb-889f-46a0-a765-e25aed4344b4
md"""
## Line plot
"""

# ╔═╡ 6cd93e76-9b81-412e-8d08-4aac01b83a6f
let
	plt = data(_df_stacked) * 
		mapping(
			:variable => "x label",
			:value => "y label",
			color = :Area => "legend label"
		) *
		visual(Lines)
...
# ╔═╡ 14f31954-e1bf-4467-b9fc-3fd161d83175
md"""
## Bar plot
"""

# ╔═╡ 80636220-8ce4-4c40-8187-4a9f43c994ac
let
	plt = data(_df_stacked) * 
		mapping(
			:variable => (x -> split(x, '_')[end]) => "x label",
			:value => "y label",
			color = :Area => "legend label",
			dodge = :Area,
		) *
	...
	draw(plt) |> as_svg # maximum kwispiness
end

# ╔═╡ 60c95dd9-adab-4692-bfb9-e73fdf38377a
md"""
## Using Python directly

This should show up in your systems's backend gui.

!!! tip
	For inline plotting directly in this notebook, check out the `#main` branch of [`PythonCall.jl`](https://github.com/cjdoris/PythonCall.jl) and use `plt.gcf()` instead of `plt.show()`
"""

# ╔═╡ c23aefb7-41ae-4a13-9af7-9e57a9c112dd
CondaPkg.add.(("matplotlib", "pandas", "numpy")); CondaPkg.resolve()
...
df = pd.DataFrame({
	"Area": ["South West", "Edinburgh", "London", "Morecombe"],
	"ER_2005": np.random.rand(4),
	"ER_2015": np.random.rand(4),
	"ER_2025": np.random.rand(4),
})

df_processed = df \
	.set_index("Area") \
	.filter(regex = "ER_.*") \
	.rename(columns = lambda s: int(s[3:])).T[["South West", "London"]]
